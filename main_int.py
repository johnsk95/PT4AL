...
# trainset = Loader2(is_train=0, transform=transform_train)
...
# Training
def train(criterion, optimizer, epoch, trainloader):
    print('\nEpoch: %d' % epoch)
    net.train()
    train_loss = 0
    correct = 0
...
def test(criterion, epoch, cycle):
    global best_acc
    net.eval()
    test_loss = 0
    correct = 0
...
def get_plabels(net, samples, cycle):
    ...
    # Check class balance of pseudolabels
    # classdist = []
